...
sys.path.append('/home/gon/Desktop/tf_playground/pitch_c')
from c_preprocessing_merge import CreateMLBYoutubeDataset
...
ROOT_PATH = '/home/gon/Desktop/sampled_video_resize' # Sub-sampled Video Folder
NEGATIVE_ROOT_PAHT = '/home/gon/Desktop/sampled_noact_resize'
...
class DataLoader(Sequence):
    ...
    def __data__generation(self, temp_data_list):
        ...
        for i, frames in enumerate(temp_data_list):
            # frames[0] = frames([16, 224, 224, 3]) / frames[1] = labels / frames[2] = mask / frames[3] = vid
            if len(frames[0][0]) < 15:
                continue
...
class ActivityRecognition(Model):
    def __init__(self, dataloader):
        ...
        self.cnn = Model(backbone_model.inputs, [backbone_model.output, backbone_model.layers[-3].output]) # Vector, Feature map
    ...
    def call(self, x, training=None):
        input, label, mask_info, vid = x
        self.cnn.trainable = training
        ...
        pooled_feature = self.max_pool(feature) # [1, 1, 2048]
        ...
        # Data Masking
        mask = tf.cast(mask_info, tf.float32) # [True : 1, False : 0]
        ...
        result = self.dense1(v_last_state)
    ...
    for epoch in range(num_epochs):
        ...
        for x in dataloader:
            ...
            with tf.GradientTape() as tape:
                ...
                # 21-04-20 / add indent
                temp_pred = tf.argmax(pred, axis=-1)
            ...
            print("{0:0d} - {1:>4d} / {2:0d}. Current Loss : [{3:0.6f}] Current Acc : [{4:0.6f}]".format(epoch, i, len(dataloader) * 2,
...
# Data pre-processing
train_dataset = CreateMLBYoutubeDataset(SPLIT_FILE_PATH, NEGATIVE_SPLIT_FILE_PATH, "training", ROOT_PATH, NEGATIVE_ROOT_PAHT, MAX_FRAME_LENGTH)
test_dataset = CreateMLBYoutubeDataset(SPLIT_FILE_PATH, NEGATIVE_SPLIT_FILE_PATH, "testing", ROOT_PATH, NEGATIVE_ROOT_PAHT, MAX_FRAME_LENGTH)
